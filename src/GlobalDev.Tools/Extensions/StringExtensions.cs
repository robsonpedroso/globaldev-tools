...
namespace GlobalDev.Tools.Extensions
{
    public static class StringExtensions
    {
        ...
        /// </summary>
        /// <param name="document">CPF or CNPJ</param>
        /// <returns>True - if document valid; False - if document not valid</returns>
        public static bool IsValidDocument(this string document)
        {
            try
            {
                document = document.ClearStrings();

                if (document.IsNullOrWhiteSpace() || (document.Length != 14 && document.Length != 11))
                    return false;
